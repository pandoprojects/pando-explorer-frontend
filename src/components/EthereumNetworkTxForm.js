...
export class EthereumNetworkTxForm extends React.Component {
    ...
    async handleEntireBalanceClick() {
        if (this.state.tokenType === TokenTypes.ERC20_PANDO) {
            let balance = this.props.balancesByType[TokenTypes.ERC20_PANDO];
    ...
    async validateAmount() {
        ...
        let erc20PandoBalance = this.props.balancesByType[TokenTypes.ERC20_PANDO];
        let ethereumBalance = this.props.balancesByType[TokenTypes.ETHEREUM];
        let balance = null;

        if (this.state.tokenType === TokenTypes.ERC20_PANDO) {
            balance = erc20PandoBalance;
        } else if (this.state.tokenType === TokenTypes.ETHEREUM) {
    ...
    render() {
        ...
        let ERC20PandoTitle = `ERC20 Pando (${this.props.balancesByType[TokenTypes.ERC20_PANDO]})`;
            ...
            <div className="TxForm TxForm--ethereum-network">
                <FormInputContainer title="Token">
                    <select className="BottomBorderInput" value={this.state.tokenType} onChange={this.handleChange}
                        name="tokenType">
                        <option value={TokenTypes.ERC20_PANDO}>{ERC20PandoTitle}</option>
                ...
                <GradientButton title="Send" className="GradientButton"
