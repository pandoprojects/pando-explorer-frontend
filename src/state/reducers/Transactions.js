...
import {isPandoNetwork} from "../../constants/Networks";
...
function pendingPandoNetworkTransactionToLocalTransaction(network, pendingTransaction, hash) {
        ...
        inputs: [
            {
                address: from,
                coins: {
                    pando: (tokenType === TokenTypes.PANDO ? amount : 0),
                    ptx: (tokenType === TokenTypes.PTX ? amount : 0) + transactionFee
                }
            }
        ],
        outputs: [
            {
                ...
                coins: {
                    pando: (tokenType === TokenTypes.PANDO ? amount : 0),
                    ptx: (tokenType === TokenTypes.PTX ? amount : 0)
                }
            }
        ],
...
function pendingEthererumNetworkTransactionToLocalTransaction(network, pendingTransaction, hash) {
    ...
    if (pendingTransaction.tokenType === TokenTypes.ERC20_PANDO) {
...
function pendingTransactionToLocalTransaction(network, pendingTransaction, hash) {
    if (isPandoNetwork(network)) {
        return pendingPandoNetworkTransactionToLocalTransaction(network, pendingTransaction, hash);
    }
    else {
        //Unknown network
        return null;
...
export const transactionsReducer = (state = INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        //Pando Transactions
        case actionTypes.FETCH_TRANSACTIONS_PANDO_START: {
            return Object.assign({}, state, {
                isFetchingTransactions: true
            });
        }
        case actionTypes.FETCH_TRANSACTIONS_PANDO_END: {
            return Object.assign({}, state, {
                isFetchingTransactions: false
            });
        }
        case actionTypes.FETCH_TRANSACTIONS_PANDO_SUCCESS: {
            let body = action.response.body;
            let transactions = body.transactions;
        ...
        case actionTypes.FETCH_TRANSACTION_SUCCESS: {
            ...
            if(isPandoNetwork(metadata.network)){
                //The explorer may give us txs that are pending and have no data.  The UI can't show these because it has no data
                transactions = _.filter(transactions, function(tx) {
                    return (tx['inputs'] !== null);
                });
