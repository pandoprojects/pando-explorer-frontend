...
function sortPandoNetworkTransactionOutputs(walletAddress, outputs) {
...
function transformPandoNetworkTransaction(walletAddress, transaction) {
    let { outputs } = transaction;
    outputs = sortPandoNetworkTransactionOutputs(walletAddress, outputs);
...
function getTransformedTransactions(walletAddress, txs, localTransactionsByHash) {
    ...
    let txTransformer = _.partial(transformPandoNetworkTransaction, walletAddress);
...
export const getERC20Transactions = createSelector(
    ...
    (walletAddress, transactionsByType, localTransactionsByHash) => {
        return getTransformedEthereumNetworkTransactions(walletAddress, TokenTypes.ERC20_PANDO, transactionsByType, localTransactionsByHash);
    }
);
...
export const getPandoNetworkTransactions = createSelector(
